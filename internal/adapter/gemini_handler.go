...
func geminiGenerateContent(c *gin.Context, pool *balancer.AccountPool, model string) {
	...
	gemini.RandomDelay()
	respBody, err := client.StreamGenerateContent(prompt, mappedModel, files, &gemini.ChatMetadata{TraceID: traceIDStr})
	if err != nil {
	...
	// 截断续写：非流式模式下阻塞重试
	completeText := AutoContinueNonStream(client, mappedModel, prompt, fullText.String())
		...
		Candidates: []GeminiCandidate{
			{
				Content: GeminiContent{
					Role:  "model",
					Parts: []GeminiPart{{Text: completeText}},
				},
				FinishReason: "STOP",
			},
		},
