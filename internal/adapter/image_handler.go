...
func ImageGenerationHandler(pool *balancer.AccountPool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for i := 0; i < req.N; i++ {
			...
			extracted := extractImagesFromResponse(respBody, req.ResponseFormat, client)
			respBody.Close()
...
func extractImagesFromResponse(reader io.Reader, format string, client *gemini.Client) []gin.H {
	var images []gin.H
	...
	for i := bodyIndex; i < len(allParts); i++ {
		...
		imgCandidate.Get("12.7.0").ForEach(func(idx gjson.Result, genImg gjson.Result) bool {
			...
			log.Printf("[Images] Found image %d URL: %s...", idx.Int(), fullSizeURL[:min(len(fullSizeURL), 60)])

			if format == "url" {
				images = append(images, gin.H{"url": fullSizeURL})
			} else {
				// 使用 gemini.Client 内置的 TLS 指纹客户端下载图片，避免绕过浏览器伪装
				data, err := client.FetchImage(fullSizeURL)
				if err != nil {
